...
import { Jimp, constants } from 'jimp'; // Corrected: Import Jimp and constants as named exports
...
async function getSsimSimilarity(imagePath1: string, imagePath2: string): Promise<number> {
  try {
    ...
    img1.resize({ w: commonSize, h: commonSize, mode: constants.RESIZE_BICUBIC });
    img2.resize({ w: commonSize, h: commonSize, mode: constants.RESIZE_BICUBIC });
